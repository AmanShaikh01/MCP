...
const ResponseDisplay = ({ response }) => {
  ...
  return (
    <div className="mt-4 text-sm bg-[#0f172a] p-4 rounded-lg border border-gray-700 max-h-60 overflow-auto">
      {parsedData.message && (
        <p className="mb-3 text-cyan-400 font-medium">{parsedData.message}</p>
      )}
        ...
        <div>
          {parsedData.students.length === 1 ? (
            parsedData.students.map((student, index) => (
              <div
                key={index}
                className="p-3 bg-[#1e293b] rounded-md border border-gray-600"
              >
            ...
            <ul className="space-y-2">
              {parsedData.students.map((student, index) => (
                <li
                  key={index}
                  className="p-2 bg-[#1e293b] rounded-md border border-gray-600 text-gray-300"
                >
                  {student.name}
                </li>
              ))}
      ...
      {parsedData.error && (
        <p className="text-red-400 font-medium">Error: {parsedData.error}</p>
      )}
    </div>
  );
};
